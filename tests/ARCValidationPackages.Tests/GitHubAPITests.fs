...
let ``GitHubAPI tests`` = 
    testList "GitHubAPI tests" [
        ...
        test "getPackageIndex contains test script" {
            ...
            Expect.isTrue (indexedPackages |> Array.exists (fun package -> package.RepoPath.Value = "src/PackageRegistryService/StagingArea/test/test@1.0.0.fsx")) "package index did not contain test script"
        }
